...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const discordBanner = document.querySelector('#discord-profile-block .banner');
  async function updateDiscordBanner() {
    ...
    for (const src of bannerCandidates) {
      try {
        const res = await fetch(src, { method: 'HEAD' });
        if (res.ok) {
          discordBanner.style.backgroundImage = `url('${src}')`;
          return;
        }
      } catch (err) {
